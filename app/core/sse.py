...
class SSEEventBus:
    ...
    def emit(self, user_id: int, event: dict[str, Any]) -> None:
    ...
    def _reap_dead_queues(self) -> None:
        """Remove queues that are full and have been for a while (abandoned consumers)."""
        stale_users = []
        for user_id, queues in self._queues.items():
            alive = [q for q in queues if q.qsize() < q.maxsize]
            if alive:
                queues[:] = alive
            else:
