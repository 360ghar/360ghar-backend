...
def get_upcoming_bookings(
    ...
    return get_user_upcoming_bookings(db, current_user.id)
...
def get_past_bookings(
    ...
    return get_user_past_bookings(db, current_user.id)
...
def cancel_booking_request(
    ...
    booking = get_booking(db, cancel_data.booking_id)
    if not booking:
        raise HTTPException(status_code=404, detail="Booking not found")
    ...
    success = cancel_booking(db, cancel_data.booking_id, cancel_data.reason)
    if not success:
...
def process_booking_payment(
    ...
    booking = get_booking(db, payment_data.booking_id)
    if not booking:
        raise HTTPException(status_code=404, detail="Booking not found")
    ...
    success = process_payment(db, payment_data)
    if not success:
...
def add_booking_review(
    ...
    booking = get_booking(db, review_data.booking_id)
    if not booking:
        raise HTTPException(status_code=404, detail="Booking not found")
    ...
    success = add_review(db, review_data)
    if not success:
